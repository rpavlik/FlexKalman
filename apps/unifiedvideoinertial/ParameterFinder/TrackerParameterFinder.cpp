...
namespace ps {
using namespace videotracker::uvbi::optimization_param_sets;
using ParamSets =
...
struct PrintParamSetOptions {
    template <typename T> void operator()(T const &) {
        std::cerr << "   "
                  << videotracker::uvbi::optimization_param_sets::ParamSetName<
                         T>::get()
                  << "\n";
    }
};
...
template <typename RefSource> class ParseArgumentAsParamSet {
  public:
    ParseArgumentAsParamSet(videotracker::uvbi::ParamOptimizerFunc &outFunc,
    ...
    template <typename T> void operator()(T const &, const char *arg) {
        using videotracker::uvbi::optimization_param_sets::ParamSetName;
        if (boost::iequals(ParamSetName<T>::get(), arg)) {
            // we've got a match!
            outFunc_ = &videotracker::uvbi::runOptimizer<RefSource, T>;
            paramSetName_ = ParamSetName<T>::get();
        }
    }

  private:
    videotracker::uvbi::ParamOptimizerFunc &outFunc_;
    std::string &paramSetName_;
};

template <typename RefSource>
int parseParamSetForParamOptimizer(videotracker::uvbi::ParamOptimizerFunc &func,
    ...
    if (argc == 2) {
        ...
        std::cout << "Will process default parameter set "
                  ...
                  << "\n";
        func =
            &videotracker::uvbi::runOptimizer<RefSource, ps::DefaultParamSet>;
        costOnly = false;
        return 0;
    }
    // OK, they specified a param set.
    videotracker::uvbi::ParamOptimizerFunc result;
    std::string paramSetName;
...
int main(int argc, char *argv[]) {
    ...
    bool costOnly = false;
    videotracker::uvbi::ParamOptimizerFunc paramOptFunc;
    {
        int ret = 0;
        switch (routine) {
        case OptimizationRoutine::ParamViaRansac:
            ret = parseParamSetForParamOptimizer<
                videotracker::uvbi::RansacOneEuro>(paramOptFunc, costOnly, argc,
                                                   argv);
            if (ret != 0) {
        ...
        case OptimizationRoutine::ParamViaRefTracker:
            ...
            ret = parseParamSetForParamOptimizer<
                videotracker::uvbi::ReferenceTracker>(paramOptFunc, costOnly,
                                                      argc, argv);
            if (ret != 0) {
                /// There was an error, and the function already told the user
                /// about it.
    ...
    auto data = videotracker::uvbi::loadData(DATAFILE);
    std::cout << "\n";

    const auto camParams =
        videotracker::getHDKCameraParameters().createUndistortedVariant();
    ...
    case OptimizationRoutine::RefTracker:
        ...
        videotracker::uvbi::computeRefTrackerTransform(
            data, videotracker::uvbi::OptimCommonData{camParams, params});
        break;

    case OptimizationRoutine::ParamViaRansac:

        paramOptFunc(data, costOnly,
                     videotracker::uvbi::OptimCommonData{camParams, params},
                     30);
        break;
