...
#define OSVR_VEC_MEMBER(COMPONENT, INDEX)                                      \
    ...
    KALMANFRAMEWORK_INLINE double osvrVec2Get##COMPONENT(OSVR_Vec2 const *v) {            \
        return v->data[INDEX];                                                 \
    }                                                                          \
    /** @brief Setter for Vec2 component COMPONENT */                          \
    KALMANFRAMEWORK_INLINE void osvrVec2Set##COMPONENT(OSVR_Vec2 *v, double val) {        \
        v->data[INDEX] = val;                                                  \
    }
...
KALMANFRAMEWORK_INLINE void osvrVec2Zero(OSVR_Vec2 *v) {
    osvrVec2SetX(v, 0);
    osvrVec2SetY(v, 0);
}
