...
inline void drawSubpixelPoint(cv::Mat image, cv::Point2d point,
    ...
    cv::circle(image, intPoint, static_cast<int>(radius * SHIFT), color, -1, 8,
               fractionalBits);
}
    ...
    for (std::size_t i = 0; i < n; ++i) {
        ...
        if (!fillContours) {
            ...
            drawSubpixelPoint(highlightedContours, data.center,
                              colorCenterDot ? color : black, centerDotRadius);
        }
    }
...
inline cv::Mat drawSingleColoredContours(
    cv::Mat const &baseImage, std::vector<ContourType> const &contours,
    cv::Scalar color, bool fillContours = false, double centerDotRadius = 1.2,
    bool colorCenterDot = true) {
    return drawFunctorColoredContours(
        baseImage, contours,
...
inline void consumeHolesOfConnectedComponents(
    cv::InputOutputArray input, std::vector<ContourType> &contoursTempStorage,
    ...
    for (int outsides = 0; outsides >= 0 && outsides < n;
         outsides =
             hierarchyTempStorage[outsides][HIERARCHY_NEXT_SIBLING_CONTOUR]) {
        for (int idx =
                 hierarchyTempStorage[outsides][HIERARCHY_FIRST_CHILD_CONTOUR];
             idx >= 0 && idx < n;
             idx = hierarchyTempStorage[idx][HIERARCHY_NEXT_SIBLING_CONTOUR])
            /// We want all first-level children of connected components.
            std::forward<F>(continuation)(std::move(contoursTempStorage[idx]));
    }
}
/// Calls a continuation on every hole of a connected component.
template <typename F>
