    ...
    class CannedIMUMeasurement {
      public:
        ...
        void restoreQuatVariance(Eigen::Vector3d &var) const {
            UVBI_ASSERT_MSG(orientationValid(), "restoring quat variance on "
        ...
        void restoreAngVel(Eigen::Vector3d &angVel) const {
            UVBI_ASSERT_MSG(angVelValid(), "restoring ang vel on "
                                           "an invalid ang vel "
                                           "measurement!");
            angVel = Eigen::Vector3d::Map(m_angVel.data());
        }
        void restoreAngVelVariance(Eigen::Vector3d &var) const {
            UVBI_ASSERT_MSG(angVelValid(), "restoring ang vel variance on "
                                           "an invalid ang vel "
                                           "measurement!");
