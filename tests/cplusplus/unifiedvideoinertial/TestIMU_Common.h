...
using namespace videotracker::uvbi;
...
struct TestData {
    TestData() {
        ...
        state.setErrorCovariance(
            flexkalman::types::DimVector<BodyState>::Constant(100).asDiagonal());
#endif
        ...
        processModel.setNoiseAutocorrelation(flexkalman::types::Vector<6>::Map(
            params.processNoiseAutocorrelation));
