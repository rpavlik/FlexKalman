...
SBDBlobExtractor::SBDBlobExtractor(BlobParams const &blobParams)
    ...
    m_sbdParams.filterByInertia = false; // Do we test for non-elongated blobs?
    ...
    m_sbdParams.filterByConvexity = p.filterByConvexity; // Test for convexity?
...
cv::Mat SBDBlobExtractor::generateDebugThresholdImage() const {
    ...
    VIDEOTRACKER_ASSERT(Algo::SimpleBlobDetector == m_algo);
...
SBDGenericBlobExtractor::SBDGenericBlobExtractor(BlobParams const &blobParams)
    : m_params(blobParams) {
...
void SBDGenericBlobExtractor::getKeypoints(cv::Mat const &grayImage) {
    ...
    // 0.3 LERP between min and max as the min threshold, but
    // don't let really dim frames confuse us.
    m_sbdParams.minThreshold = static_cast<float>(thresholdInfo.minThreshold);
