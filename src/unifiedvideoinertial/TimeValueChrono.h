    ...
    namespace time {
        class TimeValueClock {
          public:
            ...
            using rep = videotracker::util::TimeValue;
            ...
            static time_point now() {
                return time_point(duration{videotracker::util::time::getNow()});
            }
        ...
        inline videotracker::util::TimeValue
        ...
        using time_value_seconds_duration =
            std::chrono::duration<UVBI_TimeValue_Seconds>;
        /// Duration representation of the second element of the structure.
        using time_value_microseconds_duration =
            std::chrono::duration<UVBI_TimeValue_Microseconds, std::micro>;
        namespace detail {
            /// CRTP base class for things that wrap a
            /// videotracker::util::TimeValue for the purposes of some interop
            /// with std::chrono.
            template <typename Derived> class TimeValueChronoWrapperBase {
              public:
                using derived_type = Derived;

                UVBI_TimeValue_Seconds getSecondsPart() const {
                    return get().seconds;
                }
                time_value_seconds_duration getSecondsPartAsDuration() const {
                    return time_value_seconds_duration{getSecondsPart()};
                }
                UVBI_TimeValue_Microseconds getMicrosecondsPart() const {
                ...
                util::TimeValue const &get() const { return derived().get(); }
            };
        } // namespace detail
        ...
        class TimeValueChronoValueWrapper
          ...
          public:
            explicit TimeValueChronoValueWrapper(util::TimeValue const &tv)
                : m_timeval(tv){};
            util::TimeValue const &get() const { return m_timeval; }

          private:
            util::TimeValue m_timeval;
        };
        ...
        class TimeValueChronoConstRefWrapper
          ...
          public:
            explicit TimeValueChronoConstRefWrapper(util::TimeValue const &tv)
                : m_timeval(tv) {}
            /// movable
            TimeValueChronoConstRefWrapper(
          ...
          private:
            util::TimeValue const &m_timeval;
        };

        namespace detail {
            inline TimeValueChronoConstRefWrapper
            wrap_time_value(util::TimeValue const &tv) {
        ...
        inline TimeValueClock::time_point as_time_point(TimeValue const &tv) {
        ...
        to_duration(TimeValue const &tv) {
