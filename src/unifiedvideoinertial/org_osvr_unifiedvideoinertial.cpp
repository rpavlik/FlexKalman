...
using TrackingSystemPtr = std::unique_ptr<videotracker::uvbi::TrackingSystem>;
using videotracker::uvbi::BodyId;
using videotracker::uvbi::BodyReportingVector;
using videotracker::uvbi::TrackedBody;
using videotracker::uvbi::TrackedBodyIMU;
using videotracker::uvbi::TrackerThread;
...
class UnifiedVideoInertialTracker {
  ...
  private:
    ...
    videotracker::uvbi::ImageSourcePtr m_source;
    cv::Mat m_frame;
    cv::Mat m_imageGray;
  ...
  public:
    UnifiedVideoInertialTracker(OSVR_PluginRegContext ctx,
                                videotracker::uvbi::ImageSourcePtr &&source,
                                videotracker::uvbi::ConfigParams params,
        ...
        if (m_debugData) {
            osvrDeviceAnalogConfigure(
                opts, &m_analog, videotracker::uvbi::DEBUG_ANALOGS_REQUIRED);
        }
    ...
    void setupBodyReporting() {
        ...
        for (decltype(n) i = 0; i < n; ++i) {
            m_bodyReportingVector.emplace_back(
                videotracker::uvbi::BodyReporting::make());
        }
    }
    ...
    int totalNumBodies() const {
        static const auto EXTRA_TRACKING_SENSORS =
            videotracker::uvbi::extra_outputs::numExtraOutputs;
    ...
    std::string createDeviceDescriptor() const {
        ...
        Json::Value root;
        {
            ...
            if (!reader.parse(origJson, root)) {
                VIDEOTRACKER_ASSERT_MSG(
                    false, "Not possible for parsing to fail: loading "
        ...
        namespace extra_outputs = videotracker::uvbi::extra_outputs;
    ...
    void startTrackerThread() {
        ...
        m_trackerThreadManager.reset(new TrackerThread(
            *m_trackingSystem, *m_source, m_bodyReportingVector,
            videotracker::uvbi::getHDKCameraParameters(), m_camUsecOffset,
...
inline OSVR_ReturnCode UnifiedVideoInertialTracker::update() {
    ...
    for (std::size_t i = 0; i < numSensors; ++i) {
        videotracker::uvbi::BodyReport report;
        auto gotReport =
        ...
        osvrDeviceTrackerSendPoseTimestamped(m_dev, m_tracker, &report.pose, i,
                                             &report.timestamp);
        if (report.vel.angularVelocityValid || report.vel.linearVelocityValid) {
            osvrDeviceTrackerSendVelocityTimestamped(
                m_dev, m_tracker, &report.vel, i, &report.timestamp);
        }
    }
    if (m_debugData) {
        videotracker::uvbi::DebugArray arr;
...
class ConfiguredDeviceConstructor {
  public:
    ...
    OSVR_ReturnCode operator()(OSVR_PluginRegContext ctx, const char *params) {
        ...
        auto config = videotracker::uvbi::parseConfigParams(root);

#ifdef _WIN32
        auto cam = videotracker::uvbi::openHDKCameraDirectShow(config.highGain);
#else // !_WIN32
      ...
      /// other platforms instead, at least for the HDK IR camera.
        ...
        // auto cam = videotracker::uvbi::openOpenCVCamera(0);
        auto cam = videotracker::uvbi::openHDKCameraUVC();
        ...
        auto trackingSystem = videotracker::uvbi::makeHDKTrackingSystem(config);
